...
public class IntList {
    ...
    public int total() {
        ...
        while(current != null){
            sum += current.data; // append data until the end of the list
    ...
    public void switcheroo() {
        ...
        while (current != null && current.next != null) {
            ...
            current = current.next.next; // basically i+=2; node after next
        }
    ...
    public void circShiftLeft() {
        Node oldFirst = this.first; // get first node for reference

        this.first = this.first.next; // switch pointer to next node

        this.last.next = oldFirst; // append the old first node to the end of the list
        oldFirst.next = null;

        this.last = oldFirst; // update the last pointer to the old first node
    }
    ...
    private class Node {
        Node next;
        int data;
        // Default constructor for a Node - set data to -1000 (assumption is Nodes only contain positive integers)
        private Node() {
            this.next = null;
            this.data = -1000;
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("\nCircular shift Left (should print 5 11 8 15 100 6): ");
                System.out.println(myList);
        myList.circShiftLeft(); //Shift list left again
